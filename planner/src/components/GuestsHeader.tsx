...
export default function GuestsHeader({ userName, partner, items }: { userName: string; partner: string; items: SearchItem[] }) {
  ...
  const overview = path === "/guests";
  const events = path === "/guests/events";
  return (
    <>
      ...
      {events ? (
        <section className="mt-8 grid items-stretch gap-6 lg:grid-cols-[minmax(0,6fr)_minmax(0,6fr)]">
          <div className="flex flex-col justify-center">
            <p className="font-serif text-2xl font-medium">Guests</p>
            <h1 className="mt-1 font-serif text-4xl font-medium leading-[1.05] tracking-[-0.01em] sm:text-5xl xl:text-[3.5rem]">
              Let&apos;s make it a weekend to remember.
            </h1>
            <p className="mt-4 text-ink-2">From the first toast to the final coffee — all the wedding-related events in one place.</p>
          </div>
          <div className="relative min-h-[11rem] overflow-hidden rounded-2xl bg-[color-mix(in_srgb,var(--sage)_30%,var(--paper))]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/photo-candlelit-table.jpg" alt="" className="absolute inset-0 h-full w-full object-cover object-[50%_35%]" />

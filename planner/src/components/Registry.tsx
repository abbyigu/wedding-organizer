    ...
    <div className="min-h-screen pb-20 lg:pl-56">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button onClick={addEntry} className={`flex shrink-0 items-center gap-1.5 rounded-full bg-sage-deep px-4 py-2 text-sm font-semibold text-white ${FOCUS_RING}`}>
          ...
          {entries.map((e) => (
            <div key={e.id} className="rounded-2xl border border-line bg-paper p-4 shadow-sm">
              <div className="flex items-center gap-2">
                ...
                />
                {e.url && (
                  <a href={normalizeUrl(e.url)} target="_blank" rel="noreferrer" className={`shrink-0 rounded-full p-1.5 text-ink-2 hover:bg-bg hover:text-sage-deep ${FOCUS_RING}`} aria-label={`Open ${e.store_name}`}>
                    <ExternalLink className="h-4 w-4" strokeWidth={1.75} aria-hidden />
                  </a>
                )}
                <button onClick={() => removeEntry(e.id)} aria-label={`Remove ${e.store_name}`} className={`shrink-0 rounded-full p-1.5 text-ink-2 hover:bg-bg hover:text-wine ${FOCUS_RING}`}>
